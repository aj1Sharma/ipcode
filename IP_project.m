...
%log and power law transformation to compress higher pixel values
i_log=id;  %id = image double 
i_power=id;
[r c]=size(id);
c1=1.3; %log transform constant
...
%bit plane slicing
%expressing image in terms of bits
...
%adjusting image intensity to enhance contrast
hist=imhist(img);
i_eq=histeq(img);
hist_eq=imhist(i_eq);
figure;
...
%SMOOTHENING FILTER to redue noise
%Mean Filter
mf=ones(3,3)/9;
...
%SHARPENING FILTER to detect edges and apply it to the original image
%Laplacian Filter 
